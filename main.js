...
zoo.encargado = {
		...
		revisarAnimales: function(){
			...
			for(var a=0; a<listadoAnimales.length; a++){
				...
				if(animal.tieneHmabre()){
					//Reducir dinero de zoo
					animal.alimentar();
				}
			}
		},
...
var Area = function(nombreR, aforoR, recintosR, animalesR){
	this.nombre = nombreR;
	this.aforo = aforoR;
	this.recintos = recintosR;
	this.animales = animalesR;
}
...
function animales(nombre, especie, salud, hambre, pais){
	return {
		...
		ejecutarCicloAnimal: function(){
			...
			var incrementoSalud = getRandomInteger(-20, 20);
			this.modificarSalud(incrementoSalud);

		},
		estaEnfermo: function(){
			return this.salud<=50;
		},
		tieneHmabre: function(){
			return this.hambre>100;
		}
	};
}

var Animal = function(nombreR, especieR, saludR, hambreR, paisR){
		this.nombre = nombreR;
...
Animal.prototype.tieneHmabre = function(){
	return this.hambre>100;
}
...
var recinto2 = new Recinto("Baños", [], 50, "Baños para hombres y mujeres, aptos para personas con discapacidad");
